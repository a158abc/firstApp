...
function startScanSuccess(obj)
{
  ...
  {
    console.log("Scan Result：扫描结果");

    addDevice(obj.address, obj.name,obj);
  }
  else if (obj.status == "scanStarted")
  {
    console.log("Scan Started:扫描开启");
  }
  else
  {
...
{
	console.log("Retrieve Connected Success : obj.length" + JSON.stringify(obj));
...
var objArr=[];
//蓝牙扫描结果  添加蓝牙设备
function addDevice(address, name,obj)
{
	var $devices = $(".devices");
	//去重 通过address(物理地址)判断id='devices'中是否有此设备
